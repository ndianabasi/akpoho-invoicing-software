...
export default class CompanySeeder extends BaseSeeder {
  public async run() {
    ...
    await CompanyFactory.with('users', 2, (user) => user.with('profile')) // Create two random users with random access properties
      .with('users', 2, (user) => user.apply('fullAccess').with('profile')) // Two random users with full access to backend
      .with('users', 1, (user) => user.apply('companyAdmin').with('profile')) // One CompanyAdmin user with full access to backend
      .with('users', 1, (user) => user.apply('companyEditor').with('profile')) // One CompanyEditor user with full access to backend
      .with('users', 1, (user) => user.apply('companyStaff').with('profile')) // One CompanyStaff user with full access to backend
